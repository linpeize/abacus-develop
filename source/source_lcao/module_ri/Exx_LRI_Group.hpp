...
void Exx_LRI_Group<Tdata>::init(const MPI_Comm &mpi_comm_in,
						  const UnitCell &ucell,
						  const K_Vectors &kv_in,
						  const LCAO_Orbitals& orb)
{
	ModuleBase::TITLE("Exx_LRI_Group","init");
	ModuleBase::timer::tick("Exx_LRI_Group", "init");
	...
	else if(!this->info.files_abfs_group.empty())
		{ this->abfs = Exx_Abfs::IO::construct_abfs( abfs_same_atom, orb, this->info.files_abfs_group, this->info.kmesh_times ); 	}
	else
	{
		for(int T=0; T<abfs_same_atom.size(); ++T)
			this->abfs[std::to_string(T)] = abfs_same_atom[T];
	}
	Exx_Abfs::Construct_Orbs::print_orbs_size(ucell, this->abfs, GlobalV::ofs_running);

	for( const auto &abfs_T : this->abfs )
		{ GlobalC::exx_info.info_ri.abfs_Lmax = std::max( GlobalC::exx_info.info_ri.abfs_Lmax, static_cast<int>(abfs_T.size())-1 ); }
...
template<typename Tdata>
void Exx_LRI_Group<Tdata>::cal_exx_ions(const UnitCell& ucell,
								  const bool write_cv)
{
	ModuleBase::TITLE("Exx_LRI_Group","cal_exx_ions");
	ModuleBase::timer::tick("Exx_LRI_Group", "cal_exx_ions");
...
template<typename Tdata>
void Exx_LRI_Group<Tdata>::cal_exx_elec(const std::vector<std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>>& Ds,
	const UnitCell& ucell,
	const Parallel_Orbitals& pv,
	const ModuleSymmetry::Symmetry_rotation* p_symrot)
{
	ModuleBase::TITLE("Exx_LRI_Group","cal_exx_elec");
	ModuleBase::timer::tick("Exx_LRI_Group", "cal_exx_elec");
...
template<typename Tdata>
void Exx_LRI_Group<Tdata>::post_process_Hexx( std::map<TA, std::map<TAC, RI::Tensor<Tdata>>> &Hexxs_io ) const
{
	ModuleBase::TITLE("Exx_LRI_Group","post_process_Hexx");
...
template<typename Tdata>
double Exx_LRI_Group<Tdata>::post_process_Eexx(const double& Eexx_in) const
{
	ModuleBase::TITLE("Exx_LRI_Group","post_process_Eexx");
...
template<typename Tdata>
void Exx_LRI_Group<Tdata>::cal_exx_force(const int& nat)
{
	ModuleBase::TITLE("Exx_LRI_Group","cal_exx_force");
	ModuleBase::timer::tick("Exx_LRI_Group", "cal_exx_force");
...
template<typename Tdata>
void Exx_LRI_Group<Tdata>::cal_exx_stress(const double& omega, const double& lat0)
{
	ModuleBase::TITLE("Exx_LRI_Group","cal_exx_stress");
	ModuleBase::timer::tick("Exx_LRI_Group", "cal_exx_stress");
...
/*
template<typename Tdata>
std::vector<std::vector<int>> Exx_LRI_Group<Tdata>::get_abfs_nchis() const
{
